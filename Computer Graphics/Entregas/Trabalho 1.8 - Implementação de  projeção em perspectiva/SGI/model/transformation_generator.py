...
class TransformationGenerator:
    ...
    def get_perspective_projection_matrix(
        ...
        - Finalidade: Corrigir o algoritmo de projeção perspectiva, introduzindo um parâmetro explícito para o Centro de Projeção (COP) e garantindo que ele seja distinto do Ponto de Referência da Visualização (VRP), e corrigir a matriz de projeção P.
        - Prompt empregado: analise se tem algo de errado nesse algoritmo que fiz pensando no algoritmo do professor que estou  mandando por imagem. Se tiver, corrija. Talvez a cop ser a mesma que a window esteja errado. Olhe em @window.py para ver o que pode ser passado como parametro cop, caso seja esse o problema
        ***
        Retorna a matriz de projeção perspectiva.
        A composição segue:
        - Translação do VRP para a origem
        - Rotação para alinhar o sistema UVN com o sistema canônico
        - Aplicação da matriz perspectiva padrão que mapeia o frustrum para o cubo canônico.
        """
        ...
        # Passo 2: Determine os eixos do sistema de câmera (UVN)
        ...
        # Passo 3: Rotacione o mundo para alinhar o sistema UVN com o sistema de coordenadas canônico.
        R = np.identity(4)
        R[:3, 0] = u
        R[:3, 1] = v
        R[:3, 2] = vpn

        # Passo 4: Matriz de projeção perspectiva padrão.
        P = np.zeros((4, 4))
        P[0, 0] = 2 * near / window_width
        P[1, 1] = 2 * near / window_height
        P[2, 2] = -(far + near) / (far - near)
        P[2, 3] = -2 * far * near / (far - near)
        P[3, 2] = -1

        # Composição das transformações: T -> R -> P
        return P.T @ R.T @ T.T
