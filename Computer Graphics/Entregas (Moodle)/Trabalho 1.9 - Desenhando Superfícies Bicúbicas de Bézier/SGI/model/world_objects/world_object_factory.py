...
class WorldObjectFactory:
    ...
    def new_world_object(
        ...
        if object_type != WorldBezierSurface:
            if any(
                points == [(x, y, z) for x, y, z, *_ in objs.world_points]
                for objs in display_file if objs.world_points # Adicionado if objs.world_points para evitar erro com lista vazia
            ):
                return None
        else: 
            pass
        ...
        def add_object():
            if (
                ...
                if current_command == "p":
                    ...
                    obj_points = [current_object_points[i:i+4] for i in range(0, len(current_object_points), 4)]
