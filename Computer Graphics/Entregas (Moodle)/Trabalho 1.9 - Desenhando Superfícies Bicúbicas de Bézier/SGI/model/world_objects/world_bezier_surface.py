...
class WorldBezierSurface(SCWorldObject):
    ...
    def __init__(self, points: list[list[list[float]]], name: str, color: tuple[int, int, int], viewport_bounds):
        """
        @param points_matrix_4x4x3: Matriz 4x4 de pontos de controle 3D [x,y,z].
                                     Formato: [[[x,y,z], [x,y,z], ...], ...]
        """
        
        self.control_points_3d_matrix = np.array(points, dtype=float)  # matriz 4x4x3 de pontos de controle 3D [x,y,z]
        matrix_converted_to_flat = [self.control_points_3d_matrix[i, j, :].tolist() for i in range(4) for j in range(4)] # lista de pontos para WorldObject
        ...
        ***
        ...
        for i in range(4):
            for j in range(4):
                ...
                point_4d_homogeneous = np.array([point_3d[0], point_3d[1], point_3d[2], 1.0])
        ...
        for r in range(4):
            for c in range(4):
                ...
                updated_flat_points_homogeneous.append(np.array([pt[0], pt[1], pt[2], 1.0]))
        self.world_points = updated_flat_points_homogeneous


    def _calculate_surface_point_3d(self, s: float, t: float) -> tuple[float, float, float]:
            ...
            for s_idx in range(len(self.normalized_surface_grid) - 1):
                ...
                if p1_ndc is not None and p2_ndc is not None:
                    ...
                    if clipped_ndc_segment is not None:
                        viewport_points = self.transform_projection_points_to_viewport(clipped_ndc_segment)
