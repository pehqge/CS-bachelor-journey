...
class ObjectDialog(GenericDialog):
    ...
    def __init__(self):
        ...
        self.is_bezier_surface: bool = False
    ...
    def create_object(self):
        ...
        if self.is_bezier_surface:
            return self.points, self.name, self.color, None, "Bezier Surface", None
...
class BezierSurfaceDialog(GenericDialog):
        ...
        ***
        ...
        for row_str in row_strings:
            ...
            if not point_matches and row_str.strip():
                raise ValueError(f"Linha não contém pontos válidos no formato (x,y,z): '{row_str}'")
    ...
    def accept(self):
        ...
        try:
            ...
            if not all(len(row) == 4 for row in matrix) or len(matrix) != 4:
                self.show_error_message("The number of points must be 4x4.")
                return
