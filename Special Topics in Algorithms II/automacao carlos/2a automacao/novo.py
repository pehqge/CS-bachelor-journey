...
class GeminiLeadChecker:
    ...
    """
    def __init__(self, api_key, api_key_secondary, model_name="gemini-2.0-flash", stats=None):
        self.api_key = api_key
        self.api_key_original = api_key
    ...
    def switch_api_key(self):
        if self.api_key == self.api_key_original:
            self.api_key = self.api_key_secondary
        else:
            self.api_key = self.api_key_original
...
Este lead é de uma empresa que tem perfil para comprar produtos de limpeza para betoneiras?
        ...
        try:
            ...
            if "429" in str(e):
                logging.error(f"Limite de solicitações atingido. Mudando API KEY")
                self.switch_api_key()
                # if self.stats:
...
def main():
    ...
    gemini_api_key_secondary = "AIzaSyAJ4-IolrG5TN7oAwmoz8Q1Dun543DdQLA"
    ...
    gemini_checker = GeminiLeadChecker(api_key=gemini_api_key, api_key_secondary=gemini_api_key_secondary, model_name="gemini-2.0-flash-lite")
